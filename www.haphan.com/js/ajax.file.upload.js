jQuery.extend({
  ...
  createUploadForm: function (id, fileElementId, data) {
    ...
    var hdPartent = '<input type="hidden" name="{0}" value="{1}" />';
    var hdPost = "";
    for (var key in data) {
      if (data.hasOwnProperty(key)) {
        // firstProp = data[key];
        hdPost += $.Format(hdPartent, key, data[key]);
      }
    }
    //  console.log(hdPost);
    var formText = $.Format(
      '<form  action="" method="POST" name="{0}" id="{0}" enctype="multipart/form-data">{1}</form>',
      formId,
      hdPost,
    );
  ...
  ajaxFileUpload: function (s) {
    ...
    if (window.attachEvent) {
      ...
      document
        ...
        .addEventListener("load", uploadCallback, false);
    }
  ...
  uploadHttpData: function (r, type) {
    ...
    if (type == "json") {
      var i1 = data.indexOf("{", 0);
      var i2 = data.indexOf("}", 0);
      return eval("data = " + data.substring(i1, i2 + 1));
    }
