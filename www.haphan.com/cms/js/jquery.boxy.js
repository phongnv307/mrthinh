    ...
    if (node == "a") {
      jQuery(this).click(function () {
        ...
        if (active) {
          ...
          // fall back to AJAX; could do with a same-origin check
...
jQuery.extend(Boxy, {
  ...
  ask: function (question, answers, callback, options) {
    options = jQuery.extend({ modal: true, closeable: false }, options || {}, {
      show: true,
      unloadOnHide: true,
    });

    var body = jQuery("<div></div>").append(
      jQuery('<div class="question"></div>').html(question),
    );

    var buttons = jQuery('<form class="answers"></form>');
    buttons.html(
      jQuery
        .map(Boxy._values(answers), function (v) {
...
Boxy.prototype = {
  ...
  tween: function (width, height, after) {
    ...
    this.getContent()
      .stop()
      .animate({ width: bounds[2], height: bounds[3] }, function () {
        if (after) after(self);
      });
    return this;
  },
  ...
  show: function () {
    ...
    if (this.options.modal) {
      ...
      this.modalBlackout = jQuery('<div class="boxy-modal-blackout"></div>')
        .css(
          jQuery.extend(Boxy._cssForOverlay(), {
            zIndex: Boxy._nextZ(),
  ...
  _getBoundsForResize: function (width, height) {
    ...
    var p = this.getPosition();
    return [
      Math.max(p[0] - delta[0] / 2, 0),
      Math.max(p[1] - delta[1] / 2, 0),
      width,
      height,
    ];
  },
  ...
  _setupTitleBar: function () {
    if (this.options.title) {
      ...
      if (this.options.closeable) {
        tb.append(
          jQuery("<a href='#' class='close'></a>").html(this.options.closeText),
        );
      }
      if (this.options.draggable) {
        ...
        tb.mousedown(function (evt) {
          ...
          Boxy.dragging = [
            self,
            evt.pageX - self.boxy[0].offsetLeft,
            evt.pageY - self.boxy[0].offsetTop,
          ];
